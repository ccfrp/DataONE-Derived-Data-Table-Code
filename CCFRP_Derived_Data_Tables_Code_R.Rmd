...
trip.data.raw = read.csv("Raw_Data/2025_CCFRP_Dat/1-Trip Information.csv")

#3-Drift Information Table
#Included bit to make any blanks change to 'NA'
drift.data.raw = read.csv("Raw_Data/2025_CCFRP_Dat/3-Drift Information.csv", na = c("", "NA")) 

#4-Caught Fishes Table
fishes.caught.raw = read.csv("Raw_Data/2025_CCFRP_Dat/4-Caught Fishes.csv")


#Fish codes to match species code with common names
fish.codes = read.csv("Raw_Data/2025_CCFRP_Dat/Fish Species.csv")

#Length weight relationships
length.weight = read.csv("Raw_Data/CCFRP_Biomass_Conversion_2026.csv")
...
## feedback from Marine Lebrec (CalOOS) on 10/25/2024 - area name mismatch between 'Southeast Farallon Islands' and 'Farallon Islands' and mismatch between 'South Cape Mendocino' and 'Cape Mendocino'.
## 09/06/2025 - EJ changing label codes to be 'Southeast Farallon Islsands' and 'South Cape Mendocino' to avoid this mismatch

trip.data = trip.data.raw %>%
  select(c(Trip.ID, Area, Month, Day, Year.Automatic)) %>%
  dplyr::rename(Year = Year.Automatic) %>%
  mutate(Area = factor(Area, levels = c('TD','CM','TM', 'RR', 'SP',
                                        'FN','BH','AN','PL',
                                        'BL','PB','PC','CP',
                                        'AI','LB','SW','LJ'),
                       labels = c('Trinidad','South Cape Mendocino','Ten Mile', 'Reading Rock',
                                  'Stewarts Point','Southeast Farallon Islands', 
...
excluded.cells = c("TDRR", "CMMM", "CMRR", "TMMM", "TMRR", "RRMM", "RRRR",
...
for(j in need_conversion){
  species_dat <- fishes.caught.raw %>%
    ...
    filter(Year %in% c(23, 24, 25), Species.Code == j) %>% ## 2026 changes to Year %in% c(23,24, 25)
...
fishes.caught.all <- fishes.caught.all_b %>%
  ...
  mutate(a = as.numeric(a),
         b = as.numeric(b),
         Length..cm. = case_when(
    Year %in% c(07:25) ~ Fork.Length..mm./10), ## changed these all to 25 in 2026
...
## code to check for NAs in length column in fishes.caught.all

# no_len <- fishes.caught.all %>%
#   filter(is.na(Length.cm))
# 
# check1 <- fishes.caught.all_b %>%
#   filter(Drift.ID == "TMR0818241503")
...
# write.csv(length.data_reorder, file = "2007-2025_CCFRP_derived_length_table.csv",
 #                                                  row.names = FALSE)
...
2) Calculate biomass in Kg by using length weight relationships: 
...
#write.csv(AllFishVars, "CCFRP_AllFishVariables2026.csv")
...
#write.csv(AllCCFRP, "2007-2025_CCFRP_derived_effort_table.csv",row.names=FALSE)
