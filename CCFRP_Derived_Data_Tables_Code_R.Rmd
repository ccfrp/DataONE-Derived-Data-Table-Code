...
length.weight = read.csv("Raw_Data/CCFRP_Biomass_Conversion_2025.csv")
...
for(j in need_conversion){
  species_dat <- fishes.caught.raw %>%
    ...
    filter(Year %in% c(23, 24), Species.Code == j) %>% ## 2025 changes to Year %in% c(23,24)
...
fishes.caught.all <- fishes.caught.all_b %>%
  ...
  mutate(a = as.numeric(a),
         b = as.numeric(b),
         Length..cm. = case_when(
    Year %in% c(07:24) ~ Fork.Length..mm./10), ## changed these all to 24 in 2025
...
### Table export for species length data
 # write.csv(length.data_reorder, file = "2007-2024_CCFRP_derived_length_table.csv",
 #                                                 row.names = FALSE)
...
#write.csv(AllFishVars, "CCFRP_AllFishVariables2025.csv")
...
#write.csv(AllCCFRP, "2007-2024_CCFRP_derived_effort_table.csv",row.names=FALSE)
